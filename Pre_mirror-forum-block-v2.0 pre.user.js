// ==UserScript==
// @name         M系镜像站扩展-屏蔽功能升级版.加入关键词屏蔽
// @namespace    https://mirror.chromaso.net/
// @version      2.0
// @description  1.遮罩式屏蔽 美观且可临时打开 2.加入关键词屏蔽功能 3.全功能美观化管理面板
...
(function () {
  ...
  function applyAll() {
    ...
    document.querySelectorAll('.mm-post').forEach(post => {
      ...
      // B. 深度扫描：检测内容(引用)中是否含有黑名单用户
      const linksInBody = body.querySelectorAll('a[href^="/author/"]');
      for (let link of linksInBody) {
          const mentionedUser = link.textContent.trim();
          if (blockedSet.has(normalizeName(mentionedUser))) {
              executeBlock(post, body, `引用黑名单: ${mentionedUser}`);
              return; // 发现一个就拦截，跳出
          }
      }
  ...
  function updatePanel() {
    ...
    if (panelCollapsed) {
        ...
        panel.onclick = () => { panelCollapsed = false; GM_setValue('panelCollapsed', false); updatePanel(); };
        return;
    }
    ...
    con.innerHTML = `
        ...
        <div style="display:flex; gap:2px; margin-bottom:10px; background:#f1f3f5; padding:2px; border-radius:6px;">
            <div id="tab-user" style="flex:1; text-align:center; padding:5px; cursor:pointer; font-size:12px; border-radius:4px; ${activeTab==='user'?'background:#fff;font-weight:bold;':''}">用户</div>
            <div id="tab-key" style="flex:1; text-align:center; padding:5px; cursor:pointer; font-size:12px; border-radius:4px; ${activeTab==='key'?'background:#fff;font-weight:bold;':''}">关键词</div>
        </div>
        <div style="display:flex; gap:5px; margin-bottom:10px;">
            <input id="p-input" type="text" placeholder="添加${activeTab==='user'?'用户':'词'}..." style="flex:1; padding:5px; border:1px solid #ddd; border-radius:4px; font-size:12px;">
    ...
    data.forEach(item => {
        ...
        row.innerHTML = `<span style="word-break:break-all;">${item}</span><span class="del-item" style="color:red; cursor:pointer;">×</span>`;
        row.querySelector('.del-item').onclick = () => {
            if(activeTab === 'user') blockedUsers = blockedUsers.filter(x => x !== item);
            else blockedKeywords = blockedKeywords.filter(x => x !== item);
    ...
    con.querySelector('#p-close').onclick = () => { panelCollapsed = true; GM_setValue('panelCollapsed', true); updatePanel(); };
    con.querySelector('#tab-user').onclick = () => { activeTab = 'user'; updatePanel(); };
    con.querySelector('#tab-key').onclick = () => { activeTab = 'key'; updatePanel(); };
