# Mirror chromaso Forum Block Pro (v2.3 深度强化版)

一个专为 **M 系镜像论坛（mirror.chromaso.net）** 打造的深度屏蔽扩展脚本。不仅能过滤不想看的人，更能守护你的冲浪心情。

## 🚀 v2.3 重磅更新：小说标签(Tag)级精准过滤

- 🏷️ **全自动标签比对**：
  - 现在脚本会将您设定的 **“屏蔽词”** 与小说贴下方的 **标签 (#Tag)** 进行实时比对。
  - **精准避雷**：如果您讨厌某种 XP，只需将其加入屏蔽词（如“黄金”、“重口”），列表页中带有 `#黄金` 或 `#重口` 标签的小说将直接被拦截。
  - **全方位扫描**：屏蔽逻辑已覆盖：**标题文字 + 所有话题标签 + 正文内容**
...
脚本核心处理逻辑如下： 此处作图失误 帖子也可理解为详细楼层 主题=主题帖
